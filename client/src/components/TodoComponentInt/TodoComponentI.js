...
const TodoComponentI=()=>{
    ...
    useEffect(
        ()=>{
            axios.get("/todo/get-tasks").then((res)=>setData(res.data)).catch((err)=>console.log(err))
        },[data]
    )
    ...
    const changeCompletion=(itemid)=>{
        axios.get(`/todo/done-task/${itemid}`).then((res)=>{
            const temp=[...data]
    ...
    const deleteTask=(itemid)=>{
        axios.get(`/todo/delete-task/${itemid}`).then((res)=>{
            const temp=[...data]
